...
namespace render {
	...
	void ship_render(sim::Ship ship, double scale_factor) {
		sim::Vector pos = {0, 0, 0};
		pos = pos + ship.pos();
		sim::Body in_body = *ship.central_mass;
		while (!in_body.Sun) { //Untill you are at the Sun, add on the thing you are orbiting's postion to yours.
			pos = pos + in_body.pos();
			in_body = *in_body.central_mass;
		}
	...
	void orbit_render(sim::Ship ship, double scale_factor) {
		...
		glBegin(GL_LINE_LOOP);
		for(double i = 0; i < 360; ++i) {
			double semilatus = ship.orbit.semimajor*(1-(ship.orbit.eccentricity*ship.orbit.eccentricity));
			double distance = semilatus/(1 + ship.orbit.eccentricity*cos(deg_to_rad(i)));
			sim::Vector pos = {distance * cos(deg_to_rad(i)), distance * sin(deg_to_rad(i)), 0};
			sim::Vector R[3] = {{cos(ship.lan)*cos(ship.aop) - sin(ship.lan)*sin(ship.aop)*cos(ship.orbit.inclination), -cos(ship.lan)*sin(ship.aop)-sin(ship.lan)*cos(ship.aop)*cos(ship.orbit.inclination), sin(ship.lan)*sin(ship.orbit.inclination)}, {sin(ship.lan)*cos(ship.aop)+cos(ship.lan)*sin(ship.aop)*cos(ship.orbit.inclination), -sin(ship.lan)*sin(ship.aop)+cos(ship.lan)*cos(ship.aop)*cos(ship.orbit.inclination), -cos(ship.lan)*sin(ship.orbit.inclination)}, {sin(ship.aop)*sin(ship.orbit.inclination), cos(ship.aop)*sin(ship.orbit.inclination), cos(ship.orbit.inclination)}}; //Conversion matrix
			sim::Vector glob_pos = {sim::Vector::dot(R[0], pos), sim::Vector::dot(R[1], pos), sim::Vector::dot(R[2], pos)};
			while(!in_body.Sun) {
				glob_pos = glob_pos + in_body.pos();
				in_body = *in_body.central_mass;
